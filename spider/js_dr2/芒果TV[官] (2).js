var rule = {
    ...
    二级: $js.toString(() => {
        ...
        if (json.data.total === 1 && json.data.list.length === 1) {
            ...
            // 修改这里，组合日期和标题
            let displayTitle = `${data.t4} ${data.t2}`;
            d.push({
                title: displayTitle, // 使用组合后的标题
                desc: data.t2,
        ...
        let playList = [];
        d.forEach(function (it, index) {
            // 直接使用已经组合好的标题
            playList.push(`${it.title}$${it.url}`);
        });
